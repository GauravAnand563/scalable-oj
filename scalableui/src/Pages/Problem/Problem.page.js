...
function ProblemPage(props) {
  ...
  let fetchProblemById = async (problemid) => {
    ...
    let response = await fetch(
      APIRoutes.SERVER_HOST + APIRoutes.APIS.GET_PROBLEM_BY_ID + problemid,
      { method: "GET" }
    );
    ...
    <PagebaseMiddleware>
      ...
      <Routes>
        ...
        <Route
          ...
          element={
            <ProblemSolvePage
              problemDescription={problemState.problem?.description}
              problemDetails={problemState?.problem}
            />
          }
        />
        <Route
