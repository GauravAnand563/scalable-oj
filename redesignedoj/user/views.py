...
def register(request):
    try:
        ...
        if("username" in jsonData and "email" in jsonData and "password" in jsonData and "firstname" in jsonData and "lastname" in jsonData):
            ...
            ourUserModel = User.objects.create(user=newuser, rating=0, status=True, country='India', token='')
            print(newuser)
            ...
            mail_subject = 'Account Verification of '+ jsonData["email"] +'- Redesigned OJ'
            
            to_email =  jsonData["email"]
            to_name = jsonData["firstname"] + " " + jsonData["lastname"]
            emailObj = Email(to_email=to_email, subject=mail_subject, text_content=str(message), html_content='',email_group='Account Verification', content_type=False, to_name=to_name)
...
#             print("token authenticated")
#             # Do something for authenticated users.
#             return JsonResponse({'message':'stats'}, status=200)
